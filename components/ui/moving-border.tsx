    ...
    >
      ...
      >
        ...
        <MovingBorder duration={duration} rx="30%" ry="30%">
          <div
            className={cn(
              "h-20 w-20 opacity-[0.8] bg-[radial-gradient(var(--secondary)_40%,transparent_60%)]",
              borderClassName
            )}
          />
        </MovingBorder>
...
export const MovingBorder = ({
  ...
  const x = useTransform(
    progress,
    (val) => (pathRef.current as any)?.getPointAtLength(val).x
  );
  const y = useTransform(
    progress,
    (val) => (pathRef.current as any)?.getPointAtLength(val).y
  );
